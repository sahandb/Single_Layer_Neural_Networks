...
def resizeImage(location, saveLoc):
    ...
    for filename in glob.glob(location):
        ...
        imr = im.resize((622, 534))  # resize image to max one (here us 622 * 534) !!! hard code !!!
...
noisy_matrix_train, feature_train_matrix, target_train = resizeImage(dirTrain, '../pic/resized/train/SLCP/')
noisy_matrix_test, feature_test_matrix, target_test = resizeImage(dirTest, '../pic/resized/test/SLCP/')
noisy_matrix_somepics, feature_somepics_matrix, target_somepics = resizeImage(dirNoisy, '../pic/resized/somepics/SLCP/')
...
feature_somepics_vector = feature_somepics_matrix.reshape(
    (len(feature_somepics_matrix), feature_somepics_matrix.shape[1] * feature_somepics_matrix.shape[2]))
...
target_pred_somepics = ppn.predict(feature_somepics_vector)
...
# all data
print("Accuracy for all train and test")
...
print('Accuracy on somepics: %.2f' % accuracy_score(target_somepics, target_pred_somepics))
